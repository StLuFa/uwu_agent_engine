//! visual_script NodeDefinition registration — `"memory.retrieve"` (Impure + Async)
...
pub fn register_nodes(lib: &mut NodeLibrary) {
    lib.register(NodeDefinition {
        id: "memory.retrieve".into(),
        purity: Purity::Impure,
        inputs: vec![exec_in(), data_in("query", ValueType::String)],
        outputs: vec![exec_out("exec"), data_out("memories", ValueType::String)],
        runner: RunnerKind::Async(Arc::new(MemoryRetrieveRunner)),
    });
}

struct MemoryRetrieveRunner;
#[async_trait::async_trait]
impl AsyncNodeRunner for MemoryRetrieveRunner {
    async fn invoke(&self, inputs: &[Value], outputs: &mut [Value], ctx: &mut InvokeCtx<'_>) -> VsResult<ExecNext> {
        let _ = (inputs, ctx);
        outputs[1] = Value::String("retrieved memories".into());
