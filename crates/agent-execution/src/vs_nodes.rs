//! visual_script NodeDefinition registration — `"execution.act"` (Impure + Async)
...
pub fn register_nodes(lib: &mut NodeLibrary) {
    lib.register(NodeDefinition {
        id: "execution.act".into(),
        purity: Purity::Impure,
        inputs: vec![exec_in(), data_in("action_json", ValueType::String)],
        outputs: vec![exec_out("exec"), data_out("result", ValueType::String)],
        runner: RunnerKind::Async(Arc::new(ExecutionRunner)),
    });
}

struct ExecutionRunner;
#[async_trait::async_trait]
impl AsyncNodeRunner for ExecutionRunner {
    async fn invoke(&self, inputs: &[Value], outputs: &mut [Value], ctx: &mut InvokeCtx<'_>) -> VsResult<ExecNext> {
        let _ = (inputs, ctx);
        outputs[1] = Value::String(r#"{"status":"ok"}"#.into());
